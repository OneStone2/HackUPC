...
const double LARGE = 1e18;

//Aquesta versió permet a l'usuari especificar un minim i un maxim de dies per ciutat,
//aixi com decidir si les dates d'inici i final han de ser estrictes o no
...
int main() {
	int n, k;
	cin >> n >> k;
	bool strict_begin, strict_end;
	cin >> strict_begin >> strict_end;
	int l=1;
	while (l<k+1) l*=2;
	vector< pair<int,int> > req(n+1,{0,LARGE});
	...
	do {
		...
		for (int i=0;i<=n;i++) {
			...
			for (int d=0;d<=k and (i!=0 or d==0 or !strict_begin);d++) {
				dp[i][d]={g[d][a][b],0};
				if (i!=0) {
					pair<double,int> res=query(max(0,d-req[perm[i-1]].second),d-req[perm[i-1]].first+1,0,l,1);
					dp[i][d].first+=res.first;
					dp[i][d].second=res.second;
				}
			}
		}
		for (int d=k;d>=0 and (d==k or !strict_end);d--) {
